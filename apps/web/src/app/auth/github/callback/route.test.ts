...
import { upsertGithubUser, type GitHubIdentity, type User } from '../../../../modules/identity/index.js';
...
test('callback exchanges GitHub data and upserts one user without creating a session', async () => {
  ...
  assert.match(firstResponse.headers.get('set-cookie')!, /^github_oauth_state=;/);
  assert.match(firstResponse.headers.get('set-cookie')!, /Max-Age=0/);
  assert.doesNotMatch(firstResponse.headers.get('set-cookie')!, /session/i);
  ...
  assert.equal((await db.select().from(sessions).where(eq(sessions.userId, rows[0].id))).length, 0);
...
test('callback rejects missing code and invalid state without contacting GitHub', async () => {
  let calls = 0;
  const handler = createGithubCallbackHandler({
...
test('callback requires a verified primary email', async () => {
  const responses = [
...
test('callback returns 502 for token, API, and malformed provider responses', async () => {
